...
StarRocks. It is a streamhouse rebuild of the author's kappa/lambda IoT posts.

**There is deliberately no broker in the analytics path.** A Fluss log table is the append-only
partitioned stream a Kafka topic would otherwise hold, except queryable — so `iot_telemetry` is
where readings land. Kafka stays a **Tutorial-4-only** dependency (the "you cannot query a log"
contrast) and `verify.sh` does not gate on it. Do not reintroduce it upstream of `sql/07`;
`docs/EXPLANATION.md` has a "Putting a broker upstream" section for readers who want it.
